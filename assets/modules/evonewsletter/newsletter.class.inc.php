...
class newsletter {
	...
	function __construct(){
		...
		$this->_groupsTable = $this->_prefix."easynewsletter_group";
	}
...
/**
	...
	*
	**/
	public function parseTpl($tpl, $phs) {
    if (!is_array($phs)) {
      return $tpl;
    } else {
      $srchPre = array_keys($phs);
      foreach ($srchPre as $key => $value) {
        $srch[$key] = '[+'.$value.'+]';
      }
      $repl = array_values($phs);
      return str_replace($srch, $repl, $tpl);
    }
  }
	...
	*
	**/
	public function getGroupRows($tpl,$time=''){
    global $modx;
    $result = $modx->db->select('*', $this->_groupsTable, $where);
    $groupRows = '';
    while($row = $modx->db->getRow($result)) {
    	$row['countwebusers'] = 0;
    	if ($row['webusers']!='') {
    		$row['countwebusers'] = count(explode(',',$row['webusers']));
    	}
      $groupRows .= $this->parseTpl($tpl, $row);
    }
    return $groupRows;
  }
